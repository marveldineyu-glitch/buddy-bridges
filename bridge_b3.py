...
async def handle_result(m, is_edit=False):
    clean_memory()
    if m.id in _sent_ids: return
    _sent_ids.add(m.id)
    ...
    if m.text and ("no results found" in m.text.lower() or "not available" in m.text.lower()): return
    if m.media:
        if user_sessions:
        ...
        if is_edit and search_msg_id in search_results:
            try:
                await bot.edit_message(search_results[m.id][0], search_results[m.id][1], text[:4000], buttons=buttons)
                return
            except: pass
        
        if search_msg_id not in search_results:
...
async def on_result(event):
    await handle_result(event.message, is_edit=False)

@user.on(events.MessageEdited(chats=SEARCH_GROUP))
async def on_edit(event):
    await handle_result(event.message, is_edit=True)
